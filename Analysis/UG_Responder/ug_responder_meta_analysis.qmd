---
title: "Ultimatum Game (Responder)"
author: "Liam Doyle"
format: html
...
This document is a summary of the analyses performed for the sixth meta-analytic model conducted in Study 1 of my dissertation. This model examined the relationship between psychopathy and exploitative behavior in the Ultimatum Game as the Responder.
...
UG_Responder <- read_excel("UG_Responder.xlsx")
```
...
## Multi-Level Random-Effects Model of Psychopathy on Cooperation in the Ultimatum Game (Responder)

As noted in the heading and the project description, this model utilizes a multi-level or hierarchical approach to modelling the relationship between psychopathy and cooperation in the Ultimatum Game as the Responder. This is due to the fact that several of the studies included in the meta-analysis contributed more than one effect size to the analysis (e.g., a study using more than one measure of psychopathy). The contemporary recommendation for addressing this issue is to use a multi-level approach to cluster these data points - rather than simply aggregating them and ignoring potential differences between the effects.
...
As we can see, the model estimates that the meta-analytic effect size is *r* = -.043, 95% CI \[-.104, .018\]. Cochran's *Q* statistic was not significant, indicating little to no between-study heterogeneity.
...
In our case, roughly 13.01% of the variation between effect sizes can be attributed to between-study heterogeneity. The remaining variance has been attributed to sampling error. No within-study sampling variability was detected. This *I*^2^ value is considered to represent a very small amount of heterogeneity.
...
This funnel plot does possess some asymmetry, with more effect sizes expected at the bottom-left quadrant of the funnel. That being said, it should be noted that asymmetry in a funnel plot has several possible sources (Page et al., 2020):
...
title("Funnel Plot of the Effect Sizes for the UG-R Model")
```
...
The omnibus test indicates that there is no significant difference between published and unpublished studies, *F*(1, 13) = 0.02, *p* = .881. Estimates of unpublished studies, *r* = -.037, were comparable to those of published studies, *r* = -.047.
...
As seen in the output, the intercept in the model approaches a trend towards significance (*p* = 108). This indicates that a true effect does not exist outside of publication bias. According to the PET-PEESE method, we should not proceed to the PEESE step of modelling. However, the PET-PEESE method has been acknowledged by its creator (Stanley, 2017) to (a) perform poorly under conditions of small sample size (i.e., *k* \< 20) and high between-study heterogeneity (i.e., *I*^2^ \> 80) and (b) heavily over-corrects for bias in the data (Carter et al., 2019). While we have no major between-/within-study heterogeneity indicated by our *I*^2^ value, we do have fewer studies than we should (i.e., k = 15) for this analysis. As an exploratory step, PEESE analysis will be conducted.
...
More particularly, a positive slope indicates that studies with larger variances (and consequently larger standard errors, and in many cases, smaller samples) report larger effect sizes. This is an indicator of the typical "small-study effect" and indicates potential publication bias.

In contrast, a negative slope suggests that the size of study effects is inversely related to the size of study variances (e.g., studies with small effects report larger variances). This may indicate other sources of bias or sources of heterogeneity impacting the results.
...
As seen in the model output, the PEESE method indicates that the true effect size is *r* = -.099 and significant at *p* = .046. This differs from the original meta-analytic estimate, *r* = -.043, and is in fact *larger* than the original estimates.
...
```{r PEESE plot}
plot(
  UG_Responder$var_z_psyc_coop, UG_Responder$z_psyc_coop,
  xlab = "Variance of Effect Sizes",
  ylab = "Effect Sizes",
  main = "PEESE Analysis of Psychopathy-Cooperation Meta-Analysis (UG-Responder)"
)
