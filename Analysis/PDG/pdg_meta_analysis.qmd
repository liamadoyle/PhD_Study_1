...
This document is a summary of the analyses performed for the second meta-analytic model conducted in Study 1 of my dissertation. This model examined the relationship between psychopathy and exploitative behavior in the Prisoner's Dilemma Game.
...
## Multi-Level Random-Effects Model of Psychopathy on Cooperation in the Prisoner's Dilemma Game

As noted in the heading and the project description, this model utilizes a multi-level or hierarchical approach to modelling the relationship between psychopathy and cooperation in the Prisoner's Dilemma Game. This is due to the fact that several of the studies included in the meta-analysis contributed more than one effect size to the analysis (e.g., a study using more than one measure of psychopathy). The contemporary recommendation for addressing this issue is to use a multi-level approach to cluster these data points - rather than simply aggregating them and ignoring potential differences between the effects.
...
As we can see, the model estimates that the meta-analytic effect size is *r* = -.157, 95% CI \[-.222, -.093\]. Cochrane's *Q* statistic was significant, indicating potential between-study heterogeneity.
...
In our case, roughly 72.39% of the variation between effect sizes can be attributed to within-study (16.1% of the total variation) or between-study heterogeneity (56.29%). The remaining variance has been attributed to sampling error. This *I*^2^ value is considered to represent a substantial amount of heterogeneity.
...
While this funnel plot does possess some asymmetry, it should be noted that our *I*^2^ value indicated a substantial amount of heterogeneity between effect sizes (i.e., 72.39% of the variation between effect sizes was due to within-study and between-study heterogeneity). Asymmetry in a funnel plot has several possible sources (Page et al., 2020):
...
title("Funnel Plot of the Effect Sizes for the PDG Model")
```
...
The intercept was not significant, indicating that there is not a substantial amount
...
The omnibus test indicates that there is no significant difference between published and unpublished studies, *F*(1, 30) = 1.34, *p* = .257.
...
As seen in the output, the intercept in the model trends towards significance (*p* = .056). This indicates that a true effect exists. According to the PET-PEESE method, we should proceed to the PEESE step of modelling.
...
As seen in the model output, the PEESE method indicates that the true effect size is *r* = -.154 and significant at *p* \< .0001. This differs only very slightly from our original estimate of the effect at *r* = -.157.
...
```{r PEESE plot}
plot(
  PDG$var_z_psyc_coop, PDG$z_psyc_coop,
...
Note: The *k*-index indicates the degree to which the payoffs in a game incentivize cooperation or defection in a 2x2 game (e.g., the PDG). The *k*-index is bounded between 0 and 1, with 0 indicating a game where exploitation is heavily incentivized and 1 indicating a game where cooperation is heavily incentivized (Rapoport and Chammah, 1965).
