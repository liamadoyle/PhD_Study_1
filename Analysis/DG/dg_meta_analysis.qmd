...
This document is a summary of the analyses performed for the fourth meta-analytic model conducted in Study 1 of my dissertation. This model examined the relationship between psychopathy and exploitative behavior in the Dictator Game.
...
## Multi-Level Random-Effects Model of Psychopathy on Cooperation in the Dictator Game
...
As we can see, the model estimates that the meta-analytic effect size is *r* = -.171, 95% CI \[-.220, -.126\]. Cochran's *Q* statistic was significant (*p* \< .0001), indicating potential between-study heterogeneity.
...
While this funnel plot does possess a small degree of asymmetry, it should be noted that our *I*^2^ value indicated a substantial amount of heterogeneity between effect sizes (i.e., 68.5% of the variation between effect sizes was due to within-study and between-study heterogeneity). Asymmetry in a funnel plot has several possible sources (Page et al., 2020):
...
title("Funnel Plot of the Effect Sizes for the DG Model")
```
...
The omnibus test indicates that there is no significant difference between published and unpublished studies, *F*(1, 21) = 2.86, *p* = .105. That said, it is close to being a trend towards significance (i.e., *p* \< .10). The estimates that unpublished studies have a parameter of *r* = -.101, and published studies have a parameter of *r* = -.193.
...
As seen in the output, the intercept in the model is significant (*p* = .002). This indicates that a true effect exists. According to the PET-PEESE method, we should proceed to the PEESE step of modelling.
...
More particularly, a negative slope indicates that smaller studies (with larger standard errors by necessity) report larger effect sizes. This is representative of the typical "small-study effect" and indicates potential publication bias.

In contrast, a positive slope suggests that smaller studies report smaller effect sizes. This indicates other sources of bias or sources of heterogeneity impacting the results.
...
As seen in the model output, the PEESE method indicates that the true effect size is *r* = -.170 and significant at *p* \< .0001. This differs only very slightly from our original estimate of the effect at *r* = -.171.

The slope coefficient indicates a non-significant, negative relationship between effect sizes and their variances (see above). The fact that this relationship is not significant indicates that publication bias does not have a substantial impact on the effect sizes.

To illustrate the slope estimated by the model visually, we will plot the regression.

```{r PEESE plot}
plot(
  DG$var_z_psyc_coop, DG$z_psyc_coop,
  xlab = "Variance of Effect Sizes",
  ylab = "Effect Sizes",
  main = "PEESE Analysis of Psychopathy-Cooperation Meta-Analysis (DG)"
)
...
As the current model had 23 effect sizes, simple meta-regression was used to test the list of moderators. The results are shown below.
